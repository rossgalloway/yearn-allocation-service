import type { Address, AllocationSourceEvent, AllocatorDeploymentEvidence, AllocatorResolution } from './types'
...
export function resolveAllocatorAssignment(input: {
  ...
  const managerChange = events.findLast(
    (event) => event.eventName === 'UpdateRoleManager' && event.sourceAddress.toLowerCase() === vault
  )
  ...
  const assignment = events.findLast(
    (event) =>
  ...
  if (assignment.eventName === 'RemovedVault')
    return { ...base, assignmentId: assignment.id, reason: 'vault_removed_from_role_manager' }
